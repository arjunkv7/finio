...
const ACCOUNT_META: Record<AccountType, { icon: React.ComponentProps<typeof MaterialCommunityIcons>['name']; color: string; label: string }> = {
...
function AccountCard({ account, currencySymbol, onEdit, onArchive }: AccountCardProps) {
  ...
  const renderRightActions = (progress: Animated.AnimatedInterpolation<number>) => {
    ...
    return (
      <Animated.View style={[s.actions, { transform: [{ translateX }] }]}>
        <TouchableOpacity
  ...
  return (
    <Swipeable ref={swipeRef} renderRightActions={renderRightActions} overshootRight={false} friction={2}>
      <AppCard style={s.card} padding={16}>
        <View style={s.cardRow}>
          {/* Icon */}
          <View style={[s.iconWrap, { backgroundColor: hexToRgba(meta.color, 0.15) }]}>
          ...
          <View style={s.balanceCol}>
            <Text style={[s.balanceAmount, { color: isNegative ? DS.secondaryLight : DS.text.primary }]}>
...
function TransferSheet({ visible, onClose, accounts, currencySymbol, onTransfer }: TransferSheetProps) {
  ...
  return (
    <BottomSheet visible={visible} onClose={onClose} title="Transfer Funds">
      <View style={s.sheetBody}>
        ...
        <View style={s.pickerRow}>
          {activeAccounts.map((a) => {
            ...
            return (
              <TouchableOpacity
                key={a.id}
                style={[s.pickerChip, selected && s.pickerChipActive]}
                onPress={() => setFromId(a.id)}
                activeOpacity={0.7}
              >
                <MaterialCommunityIcons name={meta.icon} size={16} color={selected ? '#fff' : DS.text.muted} />
                <Text style={[s.pickerChipText, selected && s.pickerChipTextActive]} numberOfLines={1}>
                  {a.name}
                </Text>
              </TouchableOpacity>
            );
        ...
        {/* To */}
        <Text style={[s.fieldLabel, { marginTop: 16 }]}>To</Text>
        <View style={s.pickerRow}>
          {activeAccounts.map((a) => {
            const meta = ACCOUNT_META[a.type] ?? ACCOUNT_META.other;
...
export default function AccountsScreen() {
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top }]}>
      {/* ── Header ── */}
      <View style={s.header}>
        <Text style={s.headerTitle}>Wallet & Accounts</Text>
        {activeAccounts.length > 0 && (
          <View style={s.activeBadge}>
            <Text style={s.activeBadgeText}>{activeAccounts.length} Active</Text>
          </View>
        )}
      </View>
      ...
      >
        {/* ── Net Worth Hero ── */}
        <AppCard style={s.heroCard} padding={24}>
          <Text style={s.heroLabel}>Total Net Worth</Text>
          <Text style={[s.heroAmount, totalBalance < 0 && { color: DS.secondaryLight }]}>
            {totalBalance < 0 ? '−' : ''}{currencySymbol}{formattedTotal}
          </Text>
          <View style={s.heroMeta}>
            <MaterialCommunityIcons name="bank-outline" size={14} color={DS.text.muted} />
            <Text style={s.heroMetaText}>{activeAccounts.length} account{activeAccounts.length !== 1 ? 's' : ''}</Text>
          </View>
        </AppCard>
        ...
        {activeAccounts.length >= 2 && (
          ...
          >
            ...
            <Text style={s.transferBtnText}>Transfer Between Accounts</Text>
        ...
        {accounts.some((a) => a.is_archived) && (
          <View style={[s.list, { marginTop: 8 }]}>
            <Text style={s.sectionLabel}>Archived</Text>
            {accounts.filter((a) => a.is_archived).map((account) => (
              <AppCard key={account.id} style={[s.card, s.archivedCard]} padding={16}>
                <View style={s.cardRow}>
                  <View style={[s.iconWrap, { backgroundColor: hexToRgba(DS.text.muted, 0.12) }]}>
                    <MaterialCommunityIcons name="archive-outline" size={22} color={DS.text.muted} />
                  </View>
                  <View style={s.cardInfo}>
                    <Text style={[s.cardName, { color: DS.text.muted }]} numberOfLines={1}>{account.name}</Text>
                    <Text style={s.cardType}>Archived</Text>
                  </View>
                </View>
              </AppCard>
            ))}
